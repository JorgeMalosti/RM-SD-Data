...
# Definir nome do mapa e parâmetro
mapa = "Tent"  # O nome do mapa vai ser o mesmo para todos os arquivos
parametro = "μ"  # Nome do parâmetro

# Inicializar a variável para armazenar a soma das matrizes de confusão
conf_matrix_total = np.zeros((10, 10), dtype=int)

# Inicializar lista para armazenar acurácias
accuracies = []

# Definir os arquivos a serem processados (neste caso, Data_Betax_SD_1.dat até Data_Betax_SD_5.dat)
num_files = 5  # Total de arquivos (1 até 5)
for i in range(1, num_files + 1):
    # Carregar o arquivo com separador de tabulação
    file_path = f"Data_{mapa}_SD_{i}.dat"
    ...
    #print(f"Acurácia do modelo {file_path}: {accuracy:.4f}")

    # Matriz de confusão
    conf_matrix = metrics.confusion_matrix(y_test, y_pred)

    # Acumular a matriz de confusão
    conf_matrix_total += conf_matrix

# Calcular a média das matrizes de confusão
conf_matrix_average = conf_matrix_total / num_files

# Plotar a matriz de confusão média
plt.figure(figsize=(8, 6))
plt.imshow(conf_matrix_average, interpolation='nearest', cmap=plt.cm.Reds, vmin=0, vmax=60)
plt.title(f"Average Confusion Matrix - {mapa} - SD")
plt.colorbar()
...
thresh = conf_matrix_average.max() / 2.
for i in range(conf_matrix_average.shape[0]):
    for j in range(conf_matrix_average.shape[1]):
        plt.text(j, i, format(int(conf_matrix_average[i, j]), "d"),
                 horizontalalignment="center",
                 color="white" if conf_matrix_average[i, j] > thresh else "black")

plt.tight_layout()
plt.savefig(f"AverageConfusionMatrix_{mapa}.jpeg", format="jpeg")

# Salvar a acurácia média no arquivo (append)
