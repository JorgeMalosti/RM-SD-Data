...
#################################################################
# Geração da série Tent Map
#################################################################

function gerar_serie_tent(μ, tamanho, transiente)
    x = rand()
...
function main()
    ...
    for classe in 1:Classes
        μ = 1.80 + (classe * (0.19999 / Classes))
        println("Classe $classe → μ = $μ")

        for k in 1:N_series_por_classe
            serie = gerar_serie_tent(μ, data_size, transient)
            P = Patterns_Calculation(serie, Max_Patterns, data_size, Number_of_Elements, pow_vec2)
            push!(resultados, (μ=μ, probs=P, PE=Permutation_Entropy(P)))
        end
    end
...
output_file = "Data_Tent_SD_40classes_5.dat"
open(output_file, "w") do io
    write(io, "μ\tPermutation_Entropy\tPattern_Probabilities\n")
    for r in resultados
        linha = string(r.μ, '\t', r.PE, '\t', join(r.probs, ","))
        write(io, linha * "\n")
    end
end
