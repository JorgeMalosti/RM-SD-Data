...
function main()
    ...
    for classe in 1:Classes
        ...
        for k in 1:N_series_por_classe
            ...
            push!(resultados, (β=β, probs=P, PE=Permutation_Entropy(P)))
        end
    end
...
output_file = "Data_Betax_SD_40classes_5.dat"
open(output_file, "w") do io
    write(io, "β\tPermutation_Entropy\tPattern_Probabilities\n")
    for r in resultados
        linha = string(r.β, '\t', r.PE, '\t', join(r.probs, ","))
        write(io, linha * "\n")
    end
end
