...
function main()
    ...
    for classe in 1:Classes
        r = 3.60 + (classe * (0.40 / Classes))  # mesmo cálculo do Gera_Mapas_Jorge.jl
        ...
        for k in 1:N_series_por_classe
            ...
            push!(resultados, (r=r, probs=probs))
        end
    end
...
#################################################################
# Rodar e imprimir exemplo
#################################################################

resultados = main()

# Salvar resultados em arquivo .dat
output_file = "Data_Logistic_RM_40classes_1.dat"
open(output_file, "w") do io
    for r in resultados
        linha = join([r.r; r.probs], " ")
        write(io, linha * "\n")
    end
end
