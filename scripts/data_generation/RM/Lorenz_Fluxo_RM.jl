...
function main_lorenz_microstates()
    ...
    for classe in 1:Classes
        ρ = 27.99 + classe * (10.0 / Classes)  # parâmetro variável
        σ, β = 10.0, 8/3
        ...
        for k in 1:N_series_por_classe
            ...
            push!(resultados, (ρ=ρ, probs=probs))
        end
    end
...
resultados = main_lorenz_microstates()

# Salvar resultados em arquivo .dat
output_file = "Data_Lorenz_RM_20classes_1.dat"
open(output_file, "w") do io
    for r in resultados
        linha = join([r.ρ; r.probs], " ")
        write(io, linha * "\n")
    end
end
